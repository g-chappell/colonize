...
- Detailed jsdom + pure-sibling rules for Phaser code live in `docs/notes/phaser-patterns.md` (loaded via `.claude/rules/phaser-patterns.md` when editing under `apps/web/src/game/**`).
- Detailed pure-sibling rules for React component logic live in `docs/notes/ui-screens.md` (loaded via `.claude/rules/ui-screens.md` when editing `apps/web/src/**/*.tsx`).
- Registry-test relational-invariants rule lives in `docs/notes/registry-patterns.md` (loaded via `.claude/rules/registry-patterns.md` when editing under `packages/content|core/`).
...
live in `docs/notes/*.md`, loaded conditionally via path-scoped rules in
`.claude/rules/` (see index below) so they only enter context when
Claude touches a file that matches the rule's `paths` glob.
...
### Subsystem pattern notes (path-scoped)

Each bullet = note file + the rule file that loads it. Content loads only
when Claude reads files matching the rule's `paths` glob.

- TS / save-format kinds, exhaustive switches, opaque-string aliases, scalar seams, `exactOptionalPropertyTypes` traps — `docs/notes/typescript-patterns.md` (via `.claude/rules/typescript-patterns.md`, paths: `**/*.{ts,tsx}`)
- Save-format machinery (`toJSON` determinism, FIFO + re-fire-guard accumulators, defensive-copy getters) — `docs/notes/serialization-patterns.md` (via `.claude/rules/serialization-patterns.md`, paths: `packages/core|shared/**/*.ts`)
- Registry design (core-vs-content split, trim speculative fields, relational-invariant tests) — `docs/notes/registry-patterns.md` (via `.claude/rules/registry-patterns.md`, paths: `packages/content|core/**/*.ts`)
- Zustand store discipline (atomic multi-slice `set`, clamp-at-boundary) — `docs/notes/zustand-patterns.md` (via `.claude/rules/zustand-patterns.md`, paths: `apps/web/src/store/** + **/*.tsx`)
- Phaser + jsdom (React/Phaser isolation via bus, pure-sibling modules, canvas stubs) — `docs/notes/phaser-patterns.md` (via `.claude/rules/phaser-patterns.md`, paths: `apps/web/src/game/**`)
- React UI / screens (CSS Modules, click-through overlays, terminal-vs-overlay taxonomy, slice-driven event modals, heraldic inline SVG, pure-sibling for component logic) — `docs/notes/ui-screens.md` (via `.claude/rules/ui-screens.md`, paths: `apps/web/src/**/*.tsx`)
